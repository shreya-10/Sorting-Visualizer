function Insertion(){
    ...
    for(var j=0;j<array_size;j++){
        div_update(divs[j],div_sizes[j],"ffff3f");//Color update yellow
        var key= div_sizes[j];
        var i=j-1;
        while(i>=0 && div_sizes[i]>key){ 
            div_update(divs[i],div_sizes[i],"bf0603");//Color update: red
            div_update(divs[i+1],div_sizes[i+1],"bf0603");//Color update: red
